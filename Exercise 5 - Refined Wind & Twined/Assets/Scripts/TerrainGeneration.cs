...
public class TerrainGeneration : MonoBehaviour 
{
    ...
    void Start () 
	{
        ...
        //originate = new Vector3(10f, 10f, 10f); //note that the y value is irrelevant
    ...
    void Perlin(Vector3 originate, int resolution, float range)
    {
        ...
        for (int k = 0; k < resolution; k++)
        {
            for (int i = 0; i < resolution; i++)
            {
                ...
                //comment the above, then uncomment below, and compare with what happens when the WASD keys are pressed
